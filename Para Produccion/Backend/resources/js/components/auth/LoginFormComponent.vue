  ...
  <div id="login-form">
      ...
      <!-- <h1 class="title-login-center">Inicia sesion</h1> -->
        ...
        <form id="emailform" action="#" @submit.prevent="formSubmit">
          ...
          <div class="group-input" style="margin:13px auto 0px;;width: 85%;">          
            <a class="opcion-alt" href="/register">{{ trans.get('UILogin.create_new_account') }}</a>
            <a class="opcion-alt" href="iforgot/reset-password.php">{{ trans.get('UILogin.forgot_pwd') }}</a>
            <span><i class="fab fa-google google-icon"></i><a v-on:click="loadingShow" class="opcion-alt" href="/login/google" style="display:inline-block;">{{ trans.get('UILogin.login_google') }}</a></span>
          </div>
    ...
    <div v-if="stepactual == 2" class="stepslogin" id="step2">
      <div class="nav-login">
        <div id="logo-form">
          <div class="trans-black-logo-form"></div>
     <h1 class="title-login-center">{{ trans.get('UILogin.welcome_again') }}, {{this.$store.state.userdata.client_first}}</h1>
        </div>
        <form id="pwdform" action="#" @submit.prevent="login">
          <div class="group-input group-centrado">
                      ...
                      <span class="highlight"></span>
                      <span class="bar"></span>
                      <label class="label-material">{{ trans.get('UILogin.password') }}</label>
          </div>
          <div class="group-input" style="margin:13px auto 0px;width: 85%;">          
            <a class="opcion-alt" href="iforgot/reset-password.php">{{ trans.get('UILogin.forgot_pwd') }}</a>
            <a class="opcion-alt" style="cursor:pointer;" v-on:click="stepactual = 1" >{{ trans.get('pagination.back') }}</a>
    ...
    <div v-if="stepactual == 3" class="stepslogin" id="step3">
      <div class="nav-login">
        <div id="logo-form">
          <div class="trans-black-logo-form"></div>
     <h1 class="title-login-center">{{ trans.get('UILogin.msg_create_pwd') }}, {{this.$store.state.userdata.client_first}}</h1>
        </div>
        <form style ="margin:0;" id="new-pwd-form" action="#" @submit.prevent="submitPwd">
          <div class="group-input group-centrado">
          <p style="padding: 0px 22px;">{{ trans.get('UILogin.msg_create_pwd_email') }}</p>
          </div>
    ...
    <transition name="slide-horizontal">
        <button v-if="botoncontinuar" form="emailform" type="submit" class="button red login-submit" name="login-submit">{{ trans.get('pagination.next') }}</button>
        <button v-if="botonsubmit" form="pwdform" type="submit" class="button red login-submit" name="login-submit">{{ trans.get('UILogin.login') }}</button>
        <button v-if="botonPwd" form="new-pwd-form" type="submit" class="button red login-submit" name="login-submit">{{ trans.get('UILogin.create_pwd') }}</button>
    </transition>
    </div>
  </div>
</template>
  ...
  login(){
    ...
    .catch((error) => {
      ...
      if(error.response.status == 422){
        this.responseContent = this.trans.get('UILogin.invalid_pwd');
        custom.boxmsg(this.trans.get('UILogin.invalid_pwd'), 2500, toastlogin);
      }
      else if(error.response.status == 429){
        this.responseContent = "Has hecho muchos intentos seguidos en poco tiempo";
        custom.boxmsg(this.trans.get('UILogin.throttle'), 60000, toastlogin);
      }
    })
  }
  },
      ...
      if(this.username == ''){
        ...
        .then((response) => {
          ...
          if(response.data.response == 'successNotExists'){
            ...
            custom.boxtoast(this.trans.get('UILogin.email_not_exists'), this.trans.get('UILogin.not_user'), 5500, toastlogin);
